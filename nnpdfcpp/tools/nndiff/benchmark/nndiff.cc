...
real preprocdev(real const& x, real const& a, real const& b, real const& n)
{
  return -(b*n*pow(1 - x,-1 + b)*pow(x,1 - a)) + ((1 - a)*n*pow(1 - x,b))/pow(x,a);
}

real NNPDFdev(real const& x, vector<real> const& params, 
	      real const& a, real const& b, real const& n)
{
  return preprocdev(x,a,b,n)*nnval(x, params) + n*pow(x,-a+1)*pow(1-x,b)*nndiff(x, params);
}
...
real nndiff(real x_00, std::vector<real> const& params)
{
    real w_00__10 = params[0];
    real w_01__10 = params[1];
    real theta_10 = params[2];
