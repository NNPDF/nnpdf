...
void deuteron (const double& x, const double& Q, double* pdf)
{
  evolvepdf_(x,Q,pdf);
  std::swap(pdf[4], pdf[5]);
  std::swap(pdf[7], pdf[8]);
}
...
int main(int argc, char* argv[]) {
  ...
  vector<double>  xsec    = g.vconvolute(evolvepdf_, deuteron, alphaspdf_ ,nloops);
