...
def load_yaml(table_id: int, version: int = 1) -> dict:
    ...
    """
    filename = f"HEPData-ins1467454-v{version}-Table_{table_id}"
...
def get_kinematics(hepdata: dict, bin_index: list, boson: str = "Z") -> list:
    ...
    """
    del boson
    massbin = hepdata["independent_variables"][0]["values"]  # Invariant Masses
    ...
    for bins in bin_index:
        ...
        midmass = 0.5 * (mmin + mmax)

        kin_value = {
            "y": {"min": ymin, "mid": 0.5 * (ymin + ymax), "max": ymax},
            "M2": {"min": mmin**2, "mid": midmass**2, "max": mmax**2},
...
def get_data_values(hepdata: dict, bin_index: list, indx: int = 0) -> list:
    ...
    massbin = hepdata["independent_variables"][0]["values"]

    # Normalization to match APPLgrid Predictions
    return [
        2 * (massbin[i]["high"] - massbin[i]["low"]) * central[i]["value"]
        for i in bin_index
    ]
...
def get_errors(
    ...
    def extract_error(err_dict: dict) -> str:
        """Get the value of the error in the (a)symmerror case.

        Parameters
        ----------
        err_dict: dict
            dictionary containing the error value

        Returns:
        --------
        str:
            value of the error in percentage

        """
        if "symerror" in err_dict.keys():
            return err_dict["symerror"]
        elif "asymerror" in err_dict.keys():
            # Extrat the `minus` value for the percentage
            # TODO: Check if this makes a real difference
            return err_dict["asymerror"]["minus"]
        else:
            raise ValueError("Error Value not Recognised!")
    ...
    for idx in bin_index:
        stat.append(
            percentage_to_absolute(
                errors[idx]["errors"][0]["symerror"], central[idx]
            )
        )
        ...
        for sysid in range(1, len(errors[idx]["errors"]) - 1):
            sysdicts[f"sys_corr_{sysid}"].append(
                percentage_to_absolute(
                    extract_error(errors[idx]["errors"][sysid]), central[idx]
                )
            )

        sys_lumi.append(
            percentage_to_absolute(
                extract_error(errors[idx]["errors"][-1]), central[idx]
            )
        )
