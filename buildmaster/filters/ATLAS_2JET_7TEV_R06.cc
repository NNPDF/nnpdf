...
clustering algorithm for values of the jet radius parameter of 0.6, in a phase 
...
void ATLAS_2JET_7TEV_R06Filter::ReadData()
{
  ...
  for(int bin=0; bin < nbins; bin++ )
  {
    ...
    for (int i = n; i < n + ndata[bin]; i++)
    {
      ...
      fKin2[i] = (m12min + m12max)*0.5*1000.; // m12, central value of the bin (convert from TeV to GeV)
      fKin3[i] = 7000;                         // sqrt(s)
  ...
  {
    string line;
    string cov_file =  "/dijet_statcov/hepcov_R06_Eta" + to_string(bin) + ".txt";
