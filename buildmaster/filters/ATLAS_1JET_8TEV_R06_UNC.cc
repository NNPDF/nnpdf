...
N.B. In this specifica data set, uncertainties are not correlated across 
...
void ATLAS_1JET_8TEV_R06_UNCFilter::ReadData()
{
  ...
  for(int bin=0; bin < nbins; bin++ )
      ...
      for (int i = n; i < n + ndata[bin]; i++)
	{
	  ...
	  //Set all systematic uncertainties to zero
	  ...
	  //Set non-zero systematic uncertainties for individual bins 
	  for (int k=0; k<329; k++)
	    {
	      ...
	      if(tmp1>0.0 && tmp2>0.0)
		{
	      ...
	      fSys[i][2*(329*bin+k)].type = MULT;   
	      fSys[i][2*(329*bin+k)].add = sys1/sqrt(2.);
	      fSys[i][2*(329*bin+k)].mult  = fSys[i][2*(329*bin+k)].add*100/fData[i];
	      
	      fSys[i][2*(329*bin+k)+1].type = MULT;   
	      fSys[i][2*(329*bin+k)+1].add = sys2/sqrt(2.);
	      fSys[i][2*(329*bin+k)+1].mult  = fSys[i][2*(329*bin+k)+1].add*100/fData[i];
	      
	    }
	  
	  //5) Luminosity uncertainty
	  lstream >> sys1 >> sys2;
	  fSys[i][3948].type = MULT;   
	  fSys[i][3948].name = "ATLASLUMI12";   
	  fSys[i][3948].add = sys1;
	  fSys[i][3948].mult  = fSys[i][3948].add*100/fData[i];      
	}
