 ...
 * Implementation of ATLAS 2011 W minus rapidity data
 *
 * This filter contains the implementation of dataset: ATLAS_WM_7TEV,
...
// Central selection
void ATLAS_WM_7TEVFilter::ReadData()
{
  ...
  const int nBins = 1;
  const int ndataWZ[nBins] = {11};  // Data thresholds for W- 
  const double MWZ2[nBins]= {pow(MW,2.0)};   //Mass squared of W (+ and -) and (Z_low, Z_peak, Z_high)
