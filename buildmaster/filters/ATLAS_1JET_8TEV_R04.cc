...
jet clustering algorithm with radius parameter of R = 0.4 and are presented as a function
...
void ATLAS_1JET_8TEV_R04Filter::ReadData()
{
  ...
  for(int bin=0; bin < nbins; bin++ )
  {
      ...
      for (int k=0; k<319; k++)
	{
      ...
      lstream >> sys1 >> sys2;
      fSys[i][638].type = MULT;   
      fSys[i][638].name = "ATLASLUMI12";   
      fSys[i][638].add = sys1;
      fSys[i][638].mult  = fSys[i][638].add*100/fData[i];
    }
