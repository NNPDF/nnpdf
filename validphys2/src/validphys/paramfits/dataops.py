...
from reportengine.checks import CheckError, check_not_empty, check_positive, make_argcheck
from reportengine.floatformatting import ValueErrorTuple, format_error_value_columns, format_number
...
def dataspecs_matched_pseudoreplicas_chi2_table(
    ...
    return fits_matched_pseudoreplicas_chi2_table(
        dataspecs_fit, dataspecs_computed_pseudoreplicas_chi2
    )
...
def _check_badcurves(badcurves):
    ...
    if badcurves not in options:
        raise CheckError(f"badcurves must be one of {options}", badcurves, options)


def _discard_sparse_curves(fits_replica_data_correlated, max_ndiscarded):
...
def _parabolic_as_minimum_and_coefficient(
    fits_as, fits_replica_data_with_discarded_replicas, badcurves='discard'
):
...
def _parabolic_as_determination(
    fits_as, fits_replica_data_with_discarded_replicas, badcurves='discard'
):
    return _parabolic_as_minimum_and_coefficient(
        fits_as, fits_replica_data_with_discarded_replicas, badcurves
    )[0]


def quadratic_as_determination(
    fits_as, fits_replica_data_with_discarded_replicas, badcurves='discard'
):
    return _parabolic_as_minimum_and_coefficient(
        fits_as, fits_replica_data_with_discarded_replicas, badcurves
...
def _check_as_transform(as_transform):
    ...
    if not as_transform in values:
        raise CheckError(
            f"The allowed valued values for " f"as_transform are {values}",
            str(as_transform),
            values[1:],
        )
...
def _check_parabolic_as_statistics(parabolic_as_statistics):
    ...
    if parabolic_as_statistics not in values:
        raise CheckError(
            'The allowed values for' f'`parabolic_as_statistics` are {values}',
            parabolic_as_statistics,
            values,
        )
...
def as_central_parabola(fits_as, fits_total_chi2):
...
central_by_dataset_ndata = collect('ndata', ['fits_central_chi2_by_dataset_item'])
dataspecs_central_by_dataset_ndata = collect('central_by_dataset_ndata', ['dataspecs'])

by_dataset_suptitle = collect('suptitle', ['fits_matched_pseudoreplicas_chi2_by_dataset_item'])

dataspecs_dataset_suptitle = collect('by_dataset_suptitle', ['dataspecs'])


by_dataset_ndata = collect('ndata', ['fits_matched_pseudoreplicas_chi2_by_dataset_item'])
...
def as_determination_from_central_chi2_with_tag(as_determination_from_central_chi2, suptitle):
    """Convenience function to collect the arguments together. It is an identity"""
...
as_datasets_pseudoreplicas_chi2 = collect(
    parabolic_as_determination_with_tag, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)

as_datasets_central_chi2 = collect(
    as_determination_from_central_chi2_with_tag, ['fits_central_chi2_by_dataset_item']
)

parabolic_as_determination_for_total = collect(
    parabolic_as_determination, ['matched_pseudoreplicas_for_total']
)
...
quadratic_datasets_pseudoreplicas_chi2 = collect(
    quadratic_as_determination_with_tag, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)
...
@check_positive('nresamplings')
def bootstrapping_stats_error_on_the_error(
    parabolic_as_determination, nresamplings: int = 100000, suptitle=""
):
...
as_datasets_bootstrapping_stats_error = collect(
    bootstrapping_stats_error, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)

as_datasets_bootstrapping_stats_error_on_the_error = collect(
    bootstrapping_stats_error_on_the_error, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)

as_datasets_half_sample_stats_error = collect(
    half_sample_stats_error, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)
...
def compare_determinations_table_impl(pseudoreplicas_stats_error, as_datasets_central_chi2):
...
def compare_determinations_table(compare_determinations_table_impl):
    ...
    format_error_value_columns(df, ps_mean, ps_error, inplace=True)
...
dataspecs_as_datasets_pseudoreplicas_chi2 = collect(
    'as_datasets_pseudoreplicas_chi2', ['dataspecs']
)

quad_as_datasets_pseudoreplicas_chi2 = collect(
    'quadratic_datasets_pseudoreplicas_chi2', ['dataspecs']
)
...
def dataspecs_ndata_table(
    ...
    per dataset."""
    d = {}
    for dslabel, datanames, ndatas in zip(
        dataspecs_speclabel, dataspecs_dataset_suptitle, dataspecs_dataset_ndata
    ):
...
def dataspecs_quad_table_impl(
    quad_as_datasets_pseudoreplicas_chi2,
    dataspecs_speclabel,
    dataspecs_dataset_suptitle,
    dataset_items: (list, type(None)) = None,
...
def dataspecs_as_value_error_table_impl(
    dataspecs_as_datasets_pseudoreplicas_chi2,
    dataspecs_speclabel,
    dataspecs_dataset_suptitle,
    dataset_items: (list, type(None)) = None,
    display_n: bool = False,
...
by_dataset_as_chi2 = collect(
    fits_replica_data_with_discarded_replicas, ['fits_matched_pseudoreplicas_chi2_by_dataset_item']
)

dataspecs_fits_replica_data_with_discarded_replicas = collect('by_dataset_as_chi2', ['dataspecs'])


@check_not_empty('dataspecs_dataset_suptitle')
def dataspecs_chi2_by_dataset_dict(
    dataspecs_dataset_suptitle,
    ...
    for newkeys in dataspecs_dataset_suptitle[1:]:
        ...
        if symdiff:
            log.warning(f"Some datasets are not " f"common across all dataspecs {symdiff}")
            allkeys |= symdiff

    res = defaultdict(list)
    for keys, values, asvals in zip(
        dataspecs_dataset_suptitle,
        dataspecs_fits_replica_data_with_discarded_replicas,
        dataspecs_fits_as,
    ):
...
@table
def as_parabolic_coefficient_table(fits_as, by_dataset_suptitle, as_dataset_pseudodata):
