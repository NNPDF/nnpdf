...
# -*- coding: utf-8 -*-
"""
    Script to download hepdata tables given a metadata YAML file defining
    the commondata.
"""
    ...
    def __init__(self, metadata_yaml: str, path: Path=None, force: bool=False):
        ...
        if self.version != self.webversion and not self.force_download:
            sys.exit(
                ...
                f"Please use the flag --force in order to overwrite tables."
            )
...
def argument_parser():
    """Parse input arguments. The input argument should basically be the metadata."""
