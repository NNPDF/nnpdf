...
def main():
    ...
    parser.add_argument(
        "--upload-db",
        help="By default the hyperopt database is not uploaded in mongodb runs, upload it by using this option",
        action="store_true",
    )
    ...
    uploader_class = uploader_dict[input_type]

    if input_type == "hyperscan":
        uploader = uploader_class(upload_database=args.upload_db)
