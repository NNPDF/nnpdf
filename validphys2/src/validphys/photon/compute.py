...
class Photon:
    ...
    def _compute_photon_set(self):
        ...
        if self.save_to_disk:
            loader = Loader()
            path_to_photon = (
                loader._photons_qed_path / f"photon_theoryID_{self.theoryid.id}_fit_{luxset._name}"
            )
        ...
        for replica in replicas:
            ...
            if self.save_to_disk:
                np.savez_compressed(
                    path_to_photon / f"replica_{photonreplica}.npz", photon_qin=photon_qin
                )
    ...
    def _setup_fiatlux_runcard(self):
        ...
        if "eps_base" in self.lux_params:
            ...
            log.info(
                f"Using default value for fiatlux parameter eps_base = {fiatlux_runcard['eps_base']}"
            )
    ...
    def _evolve(self):
        ...
        with EKO.read(path_to_eko_photon) as eko_photon:
            ...
            if not np.isclose(self.q_in, np.sqrt(eko_photon.mu20)):
                log.error(
                    f"Photon q_in {self.q_in} does not match the one in the EKO {np.sqrt(eko_photon.mu20)}"
                )
                    ...
                    for j, pid in enumerate(basis_rotation.flavor_basis_pids):
                        if pid == 22:
                            ...
                            pdfs_init[j] = np.array(
                                [
                                    self.luxpdfset.xfxQ(x, self.q_in, photonreplica, pid) / x
                                    for x in XGRID
                                ]
                            )
...
def compute_photon_to_disk(theoryid, fiatlux, maxcores):
    ...
    def wrapper_fn(replica, theoryid, fiatlux, force_compute):
        _ = Photon(
            ...
            fiatlux,
            replicas=[replica],
            save_to_disk=force_compute,
            force_computation=force_compute,
        )
        return None

    log.info(f"Starting computation of the photon using {maxcores} effective cores...")
    _ = Parallel(n_jobs=maxcores)(
        delayed(wrapper_fn)(
            replica=replica, theoryid=theoryid, fiatlux=fiatlux, force_compute=force_compute
        )
        for replica in replicas
    )
