...
class CoreConfig(configparser.Config):
    ...
    def parse_data(self, data_or_str: (list, str), *,
            theoryid,
            use_cuts,
            fit=None,
        ...
        if isinstance(data_or_str, str):
            if data_or_str != "fromfit":
                raise ConfigError(
                    f"Invalid `data` setting: {data_or_str}.",
                    data_or_str,
                    ["fromfit", "list of dataset names or mappings with at least `dataset`"])
            if not fit:
                raise ConfigError(
                    "Setting 'data' to 'fromfit' requires "
                    "specifying a fit on which filter "
                    "has been executed, e.g.\nfit : NNPDF30_nlo_as_0118")
            data = self.loader.check_fit_data(fit)
        else:
            data = data_or_str

        if not isinstance(data, list):
            raise ConfigError(
                    f"Invalid `data` setting: {data}.",
                    data,
                    ["fromfit", "list of dataset names or mappings with at least `dataset`"])
        datasets = []
        dsinputs = []
        for item in data:
            if isinstance(item, str):
                dsinput = self.loader.check_dataset_defaults(item, theoryid.id)
            else:
                dsinput = self.parse_dataset_input(item)
            cd = self.produce_commondata(
                dataset_input=dsinput,
                use_fitcommondata=use_fitcommondata,
                fit=fit)
            cutinp = self.produce_cuts(
                commondata=cd,
                use_cuts=use_cuts,
                q2min=q2min,
                w2min=w2min,
            ...
            datasets.append(self.produce_dataset(
                dataset_input=dsinput,
                theoryid=theoryid,
                cuts=cutinp,
                fit=fit,
                ...
                use_fitcommondata=use_fitcommondata))
            dsinputs.append(dsinput)
        #TODO: change experimentspec
