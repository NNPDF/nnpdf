...
class CoreConfig(configparser.Config):
    ...
    def parse_data(self, datalist, *,
        ...
        """parse the data from the runcard or something"""
        dsinputs = [self.parse_dataset_input(ds) for ds in datalist]
        ...
        return ExperimentSpec(name='data', datasets=datasets, dsinputs=dsinputs)

    def produce_data(
            self, *, experiments=None):
        """Produces `data` keyword from experiments to retain backwards compatibility"""
        if experiments is None:
            #TODO: raise error in a better way - like `data` wasn't specified in runcard
            raise ConfigError("data not specified in runcard")
        log.warning("`experiments` keywords as been deprecated, use `data`.")
        dsinputs = [dsinput for exp in experiments for dsinput in exp]
        dsets = [ds for exp in experiments for ds in exp.datasets]
        return ExperimentSpec(name='data', datasets=dsets, dsinputs=dsinputs)
