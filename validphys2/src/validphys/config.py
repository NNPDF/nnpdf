...
class CoreConfig(configparser.Config):
    ...
    def produce_loaded_fit_covmat(self, output_path, data_input, diagonal_basis=True):
        """
        Loads the fit covmat (including the theory covmat if present) as saved during vp-setupfit
        """

        # the csv includes both the experimental and theory covmat (if requested in the runcard)
