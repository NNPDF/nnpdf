...
def frame_center(ax, x, values):
    ...
    close_anchor_min, close_anchor_max = expand_margin(close_anchor_min, close_anchor_max, 1.4)
    ...
    medium_anchor_min, medium_anchor_max = expand_margin(medium_anchor_min, medium_anchor_max, 1.1)
...
def barplot(values, collabels, datalabels, orientation='auto'):
    ...
    if lc != l:
        raise ValueError(
            f"Mismatch between the number of data points ({l}) and "
    ...
    if orientation == 'vertical':
        ...
        def get_pos(val):
            ...
            return {'xytext': xytext, 'horizontalalignment': horizontalalignment}
...
def plot_horizontal_errorbars(cvs, errors, categorylabels, datalabels=None, xlim=None):
    ...
    for cv, err, lb, markerspec, shift in zip(cvs, errors, datalabels, mi, pos):
        ax.errorbar(cv, y + shift, xerr=err, linestyle='none', label=lb, **markerspec)
    if xlim is None:
        # Support both single error and up, low.
        errmat = np.atleast_2d(errors)
        low = cvs - errmat[0, :]
        high = cvs + errmat[-1, :]
        xlim = expand_margin(np.nanpercentile(low, 15), np.nanpercentile(high, 85), 1.5)
