...
def ht_parametrisation(delta_h: list, nodes: list, x: list, Q2: list):
...
def DIS_F2R_ht(experiment, pdf, H2p_dict, H2d_dict, x, q2):
...
def DIS_F2C_ht(H2p_dict, H2d_dict, x, q2):
    # Iron target
    Z = 23.403
    A = 49.618
...
def DIS_NC_ht(H2_dict, HL_dict, x, q2, y):
    yp = 1 + np.power(1 - y, 2)
    yL = np.power(y, 2)
    N_L = -yL / yp
...
class DIS_SNU:
    def __init__(self, HT_dict, target_tuple, kin_tuple, Mh, Mw, lepton):  # (A,Z)
        ...
        self.N = (
            GEV_CM2_CONV * (GF**2) * Mh / (2 * np.pi * np.power(1 + q2 / self.Mw2, 2)) * self.yp
        )
...
class DIS_HERA_CC(DIS_SNU):
    def __init__(self, HT_dict, kin_tuple, Mh, Mw, lepton):
