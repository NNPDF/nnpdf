...
def plot_chi2(axis, one_over_k, delta_chi2, std, coeff, nrep, txt_dir):
    ...
    axis.errorbar(x, y, yerr=yerr, fmt='.--', label=label, color=color)#, elinewidth=1.5, capsize=2., capthick=1.5)
...
parser.add_argument('--num_plot', help='1: 100-125-150-175 nrep, 2: 200-250-300-350 nrep'
...
for j, txt_dir in enumerate(sort_dir):
    ...
    for i, f in enumerate(files):
        ...
        one_over_sigfrac, cv, std = np.loadtxt('../dat/' + txt_dir.name + '/' + f.name, unpack=True,
                                                usecols=(1, 2, 3))
