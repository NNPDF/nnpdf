...
def coeff_n_indep(a_n, b_n, c_n, d_n, nrep, sign=None):
    ...
    c = c_n / (_G(3., nrep) + 3/nrep * _G(1., nrep))
    if sign is not None:
        c *= -1
...
def coeff_n_dep(a, b, c, d, nrep, sign=None):
    ...
    c_n = c * (_G(3., nrep) + 3/nrep * _G(1., nrep))
    if sign is not None:
        c_n *= -1
...
def model_std(a, b, c, d, nrep):
    ...
    """
    std_a_n = a * np.sqrt(1 - _G(1., nrep)**2)

    std_b_n = b * np.sqrt(2 * (3*nrep - 2) / (nrep * (nrep-1)))

    std_c_n = c * np.sqrt((nrep**3 + 19 * nrep**2 + 3* nrep - 15) / (nrep * (nrep-1)**2)
                            - (_G(3., nrep) + 3/nrep * _G(1., nrep))**2)

    std_d_n = d * np.sqrt((nrep + 1) * (nrep + 3) * (nrep + 5) / (nrep - 1)**3
                            + 28 * (nrep + 1) * (nrep + 3) / (nrep * (nrep-1)**2)
                            + 140 * (nrep + 1) / (nrep**2 * (nrep - 1)) + 540 / nrep**3
