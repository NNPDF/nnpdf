...
parser.add_argument("--show", action="store_true", help="show delta chi2 and coefficients plots")
...
for i, f in enumerate(files):
    ...
    one_over_sigfrac, cv, std = np.loadtxt(txt_dir + "/" + f.name, unpack=True, usecols=(1, 2, 3))
...
for i, axis in enumerate(np.ndarray.flatten(ax)):
    ...
    axis.set_xscale("log")
    axis.grid(linewidth=0.2, which="both")
