...
def convolution_results_implement(data):
    ...
    covs = [results.experiment_covariance_matrix(exp, False) for exp in exps]
    return [results.experiment_results(exp, pdf, cov, pdf) for exp, cov in zip(exps, covs)]
...
def dataset_t0_convolution_results(data):
    ...
    covs = [results.covariance_matrix(x, False) for x in ds]
    return [results.results(x, pdf, cov, t0set=pdf) for x, cov in zip(ds, covs)]
...
def dataset_convolution_results(single_exp_data):
    ...
    covs = [results.covariance_matrix(ds, False) for ds in exp.datasets]
    return [results.results(ds, pdf, cov, pdf) for ds, cov in zip(exp.datasets, covs)]
