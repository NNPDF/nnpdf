    ...
    if withshifts:
        for i, (result, cuts) in enumerate(zip(results, cutlist)):
            ...
            if np.any(uncorrE == 0):
                ...
                A = np.diag(np.diag(np.ones((Nsys, Nsys)))) + np.einsum(
                    "ik,il,i->kl", corrE, corrE, 1.0 / uncorrE ** 2
                )  # eq.86
                f2 = np.einsum(
                    "kl,il,i->ik", np.linalg.inv(A), corrE, 1.0 / uncorrE
                )  # second part of eq.85
...
def plot_fancy(
    ...
    """
    if withshifts:
        one_or_more_results, shifted = get_shifted_results(
            results=one_or_more_results, commondata=commondata, cutlist=cutlist
        )
...
def plot_fancy_dataspecs(
    ...
    if withshifts:
        results, shifted = get_shifted_results(
            results=results, commondata=commondata, cutlist=cutlist
        )
        for ilabel in range(len(labellist)):
            if ilabel == 0:
                continue
...
def _check_display_cuts_requires_use_cuts(display_cuts, use_cuts):
    check(
        not (display_cuts and use_cuts not in (CutsPolicy.FROMFIT, CutsPolicy.INTERNAL)),
...
def _check_highlights(groups_data, highlight_datasets):
    if highlight_datasets:
        ...
        if diff:
            raise CheckError(f"The following highlight elements are " "not dataset names: {diff}")
