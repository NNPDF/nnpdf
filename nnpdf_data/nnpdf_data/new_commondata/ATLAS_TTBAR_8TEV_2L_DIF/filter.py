...
def processData():
    ...
    artUnc_dSig_dmttBar = covmat_to_artunc(len(values), covMatArray_dSig_dmttBar)
    ...
    for i in range(len(values) * len(values)):
        covMatArray_dSig_dmttBar_norm.append(
            input2['dependent_variables'][0]['values'][i]['value'] * 1e-6
        )
    artUnc_dSig_dmttBar_norm = covmat_to_artunc(len(values), covMatArray_dSig_dmttBar_norm, 1)
    ...
    artUnc_dSig_dyttBar = covmat_to_artunc(len(values), covMatArray_dSig_dyttBar)
    ...
    artUnc_dSig_dyttBar_norm = covmat_to_artunc(len(values), covMatArray_dSig_dyttBar_norm, 1)
