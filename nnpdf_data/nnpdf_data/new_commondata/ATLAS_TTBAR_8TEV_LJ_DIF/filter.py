...
def processData():
    ...
    for i in range(len(values)):
        ...
        for j in range(1, len(input['dependent_variables'][1]['values'][i]['errors']) - 1):
            if 'symerror' in input['dependent_variables'][1]['values'][i]['errors'][j]:
                error_value[
                    input['dependent_variables'][1]['values'][i]['errors'][j]['label'].replace(
                        " ", ""
                    )
                ] = percentage_to_absolute(
                    input['dependent_variables'][1]['values'][i]['errors'][j]['symerror'],
                    data_central_value,
                )
            else:
                se_delta, se_sigma = symmetrize_errors(
                    percentage_to_absolute(
                        input['dependent_variables'][1]['values'][i]['errors'][j]['asymerror'][
                            'plus'
                        ],
                        data_central_value,
                    ),
        ...
        error_value['lumi'] = percentage_to_absolute(
            values[i]['errors'][2]['symerror'], data_central_value
        )
    ...
    for i in range(25):
        error_definition_dSig_dmttBar['ArtUnc_' + str(i + 1)] = {
            ...
            'treatment': 'ADD',
            'type': 'ATLAS8TEVTTB151104716unc' + str(i + 1),
        }
    for i in range(1, len(input['dependent_variables'][1]['values'][0]['errors']) - 1):
        error_definition_dSig_dmttBar[
            input['dependent_variables'][1]['values'][0]['errors'][i]['label'].replace(" ", "")
        ] = {'definition': '', 'treatment': 'MULT', 'type': 'CORR'}
