...
def load_yaml(table_id: int, version: int = 1) -> dict:
    ...
    """
    filename = f"HEPData-ins1303905-v{version}-Table_{table_id}"
...
def main_filter() -> None:
    ...
    observable_tables = {"T-Y-NORM": [17, 34, 26], "TBAR-Y-NORM": [18, 35, 27]}
    ...
    # TCHANNEL-XSEC RATIO
    yaml_content_data = load_yaml(table_id=3, version=2)
    yaml_uncertainties = load_yaml(table_id=2, version=2)
    ...
    sys_types = {"treatment": ["MULT"] * n_sys, "type": ["UNCORR"] * (n_sys - 1) + ["ATLASLUMI11"]}
