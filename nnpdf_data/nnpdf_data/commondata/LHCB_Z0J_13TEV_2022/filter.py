...
uncertainties = {
    "Stat. unc": {
        "description": "Total (correlated) statistical uncertainty with correlation matrix.",
        "treatment": "ADD",
        "source": "statistical",
        "type": "CORR",
    ...
    "Eff(%)": {
        "description": "Correlated uncertainties from the selection efficiencies with correlation matrix.",
        "treatment": "MULT",
        "source": "systematic",
        "type": "CORR",
    ...
    "BKG(%)": {
        "description": "Background contributions from heavy flavours, misidentified hadrons and physics processes.",
        "treatment": "MULT",
        "source": "systematic",
    ...
    "FSR(%)": {
        "description": "Correlated uncertainties from final state radiation corrections.",
        "treatment": "MULT",
        "source": "systematic",
        "correlation fraction": 0.50,
        # "type":"CORR",
    ...
    "Closure(%)": {
        "description": "Correlated uncertainties from closure test.",
        "treatment": "MULT",
        "source": "systematic",
        "correlation fraction": 0.50,
        # "type":"CORR",
    ...
    "Alignment(%)": {
        "description": "Correlated uncertainties from detector alignment and momentum scale calibration.",
        "treatment": "MULT",
        "source": "systematic",
        "correlation fraction": 0.50,
        # "type":"CORR",
    ...
    "Unfold(%)": {
        "description": "Correlated uncertainties due to unfolding corrections applied only to pT.",
        "treatment": "MULT",
        "source": "systematic",
        "label": "Sys_unfold",
        "type": "UNCORR",
    ...
    "Luminosity": {
        "description": "Systematic uncertainty from the luminosity.",
        "treatment": "ADD",
        "source": "systematic",
        "label": "Luminosity",
        "type": "LHCB_LUM",
    ...
    for l, obs in enumerate(filtered_obs):
        ...
        for data_kin2 in data[obs]['dependent_variables']:
            ...
            for j in range(len(values)):
                ...
                error_diag_bin[uncertainties['Stat. unc']['label']] = float(
                    values[j]['errors'][0]['symerror']
                )

                # Luminosity (systematic) uncertainty
                error_diag_bin[uncertainties['Luminosity']['label']] = float(
                    values[j]['errors'][2]['symerror']
                )
                ...
                for sys_error in sys_errors:
                    ...
                    if sys_error['header']['name'] in uncertainties:
                        error_name = str(
                            sys_error['header']['name']
                        )  # Name of the uncertainty in the HepData table
                        error_label = uncertainties[error_name][
                            'label'
                        ]  # Name of the uncertainty in the 'uncertainties' dictionary
                        ...
                        if not bool(uncertainties[error_name]['absolute']):
                            ...
                            diag_val = sys_error['values'][j]['value']
                        ...
                        if 'correlation fraction' in uncertainties[error_name]:
                            ...
                            error_diag_bin[error_label + '_uncorrelated'] = float(
                                np.sqrt(1 - corr_fr) * diag_val
                            )
                            error_diag_bin[error_label + '_correlated'] = float(
                                np.sqrt(corr_fr) * diag_val
                            )
                        else:
                        ...
                        raise Exception(
                            "There is a mismatch between the dictionary 'uncertainties' and the list of systematic uncertainties provided in the HepData table(s)."
                        )
        ...
        for unc in uncertainties.values():
            if 'correlation matrix' in unc:
                ...
                for k in range(len(error_diag)):
                    ...
                    err_def_commondata[unc['label'] + f"_{k+1}"] = {
                ...
                if "correlation fraction" in unc:
                    ...
                    err_def_commondata[unc['label'] + '_corr'] = {
                        'description': unc['description'] + '(correlated)',
                        'treatment': unc['treatment'],
                        'type': 'CORR',
                    }
                    err_def_commondata[unc['label'] + '_unc'] = {
                        'description': unc['description'] + '(uncorrelated)',
                        'treatment': unc['treatment'],
                        'type': 'UNCORR',
                    }
                    # Save the uncertainty for each bin
                    for k in range(len(error_diag)):
                        error_commondata[k][unc['label'] + '_corr'] = error_diag[k][
                            unc['label'] + '_correlated'
                        ]
                        error_commondata[k][unc['label'] + '_unc'] = error_diag[k][
                            unc['label'] + '_uncorrelated'
                        ]

                else:
                    # Save definition of the uncertainty
                    err_def_commondata[unc['label']] = {
                        'description': unc['description'],
                        'treatment': unc['treatment'],
                        'type': unc['type'],
                    }
