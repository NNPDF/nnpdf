...
def magic(table, ndat, var_name, index):
    ...
    for i in range(ndat):
        ...
        if 'asymerror' in values[i]['errors'][1]:
            data_sigma, error_value['sys'] = se(values[i]['errors'][1]['asymerror']['plus'], values[i]['errors'][1]['asymerror']['minus'])
            data_central_value = data_central_value + data_sigma
        else:
