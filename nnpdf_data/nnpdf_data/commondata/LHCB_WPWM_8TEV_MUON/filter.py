...
TABLES = {2: [0], 1: [0, 2]}  # {table_id: [indexes]}
NB_POINTS = [18, 8, 8]  # [N_Z, N_W+, N_W-]
...
def read_corrmatrix(nb_datapoints: int, version: int = 1) -> np.ndarray:
    ...
    """
    corrmat = pd.read_csv(
        f"./HEPData-ins1406555-v{version}-yaml/corrmat.corr",
...
def format_uncertainties(uncs: dict, artunc: np.ndarray, bslice: slice) -> list:
    ...
    bslice: slice
        slice that selects the subtables corresponding to the Boson
...
def main_filter(boson: str = "Z") -> None:
    ...
    bslice = slice(18) if boson == "Z" else slice(18, nbpoints + 1)
