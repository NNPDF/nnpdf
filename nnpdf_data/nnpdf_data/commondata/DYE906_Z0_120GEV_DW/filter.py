...
class E906_DW_RATIO_commondata(commondata):
   def __init__(self, data: pd.DataFrame, dataset_name: str, process: str):
      ...
      artunc = pd.DataFrame(covmat_to_artunc(len(covmat), 
                                covmat.flatten().tolist()))
      artunc_names = []
      for i in range(1,len(artunc)+1): artunc_names.append(f"sys_unc_{i}")
      ...
      norm = np.sqrt(nrep)
      norm = 100
      dfN, dfp = nuclear_uncert_dw("rawdata/nuclear_ite/output/tables/group_result_table.csv",
...
def main():
   ...
   DYE906.write_new_commondata(Path("data_reimplemented_PDXSECRATIO.yaml"),
