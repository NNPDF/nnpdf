...
def check_unc_with_legacy(observable):
    logging.info(f"Comparing the new uncertainty implementation with the legacy version for {observable} using rtol = {1e-03}.")
    ...
    for i, (new_unc, legacy_unc) in enumerate(zip(new_uncs['bins'], legacy_uncs['bins'])):
      for unc, new_val, old_val in zip(new_unc.keys(), new_unc.values(), legacy_unc.values()):
        try:
          ...
          logging.warning(f"Houston, we have a problem, {new_val} != {old_val} in bin: {i+1}, unc: {unc}")


def check_dat_with_legacy(observable):
    logging.info(f"Comparing the new central data implementation with the legacy version for {observable} using rtol = {1e-03}.")
    ...
    for i, (new_val, legacy_val) in enumerate(zip(new_data['data_central'], legacy_data['data_central'])):
        try:
