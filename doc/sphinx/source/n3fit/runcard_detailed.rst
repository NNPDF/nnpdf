...
- ``log_level``: level of the ``n3fit`` logger. One of ``debug``, ``info``,
  ``warning`` or ``error``.
- ``printeach``: each how many epochs the training/validation stats (loss and
  validation :math:`\chi^2` broken down by experiment) are printed.
