...
def print_stats(dataframe, 
    ...
    for val_0, info  in decision_tree.items():
        ...
        if verbose: print("For {0}, accepted options are: {1}={2}".format(val_0, key_1, options_1))
    ...
    for val_0, item_0 in final_decision_tree.items():
        if verbose: print("For {0}".format(val_0))
        final_answer[val_0] = {}
        for val_1, item_1 in item_0.items():
            ...
            print("""
For {0} = {1}
    {2} = {3}""".format(key_0, val_0, key_1, val_1))
    ...
    key_priority_raw = ["optimizer", "epochs", "number_of_layers", "learning_rate", "activation", "layer_1", "layer_2", "layer_3"]
    ...
    from HyperAlgorithm import HyperAlgorithm
    hyperalg = HyperAlgorithm(key_info = key_info, fail_threshold=65, how_many_to_kill = how_many_to_kill, remove_by_key = remove_by_key )
...
# Wrapper
def generate_scan_report_from_file(replica_path, json_name = "tries.json", include_failures = False, histogram = None, stats = False, **kwargs):
    """
...
if __name__ == "__main__":
    ...
    for json_path in all_json:
        ...
        generate_scan_report_from_file(replica_path,
