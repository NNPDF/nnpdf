...
# Helper functions
def generate_input_DY(flavs = 3, xsize = 2, ndata = 4, basis = -1):
    """ Generates fake input for the hadronic convolution """
    # If basis = -1 combinations will include al possible combinations
    af = np.arange(flavs)
    all_combinations = np.array(np.meshgrid(af, af)).T.reshape(-1,2)
    # See whether we want to only keep some of the combinations
    if basis > 0:
        bchoice = sorted(np.random.choice( np.arange(flavs*flavs), size = basis, replace = False ))
...
def generate_input_DIS(flavs = 3, xsize = 2, ndata = 5, basis = -1):
    """ Generates fake input for the DIS convolution """
    all_combinations = np.arange(flavs)
    # Now see whether we want to only keep some of the combinations
    if basis > 0:
        combinations = sorted(np.random.choice( all_combinations, size = basis, replace = False ))
        lc = basis
    else:
        combinations = all_combinations
        lc = len(combinations)
...
# Tests
def test_DIS_basis():
    fk, comb = generate_input_DIS(flavs = FLAVS, xsize = XSIZE, ndata = NDATA, basis = FLAVS-1)
...
def test_DY_basis():
    fk, comb = generate_input_DY(flavs = FLAVS, xsize = XSIZE, ndata = NDATA, basis = FLAVS)
...
def test_DY():
    # Input values
    fk, comb = generate_input_DY(flavs = FLAVS, xsize = XSIZE, ndata = NDATA, basis = FLAVS-1)
    pdf = np.random.rand(XSIZE, FLAVS)
    kp = op.numpy_to_tensor(pdf)
    # generate the n3fit results
    obs_layer = layers.DY(NDATA, fk, basis = comb, nfl = FLAVS)
