...
from n3fit.scripts.n3fit_exec import DebugOptions
...
class ModelTrainer:
    ...
    def __init__(
        ...
        ----------
            ...
            debug_options: None, DebugOptions
                a :class:`n3fit.scripts.n3fit_exec.DebugOptions` object as parsed from the runcard
                (see docs for the meaning of each field)
        ...
        self.debug = debug
        self._debug_options = debug_options or DebugOptions()
        self.all_datasets = []
        self._scaler = None
        ...
        self.callbacks = []
        if debug or self._debug_options.timer:
            self.callbacks.append(
                callbacks.TimerCallback(
                    count_range=self._debug_options.printeach,
                    print_logs=self._debug_options.print_logs,
                )
            )

    def set_hyperopt(self, hyperopt_on, keys=None):
        """Set hyperopt options on and off"""
        if keys is None:
            keys = []
        self._hyperkeys = keys
        self.mode_hyperopt = hyperopt_on
    ...
    def _model_generation(self, xinput, pdf_model, partition, partition_idx):
        ...
        if self._debug_options.print_summary and not self.mode_hyperopt:
            training.summary()
    ...
    def _train_and_fit(self, training_model, stopping_object, epochs=100) -> bool:
        ...
        """
        callback_st = callbacks.StoppingCallback(
            stopping_object, log_freq=self._debug_options.printeach
        )
