...
#             self.validation['output'] = self.training['output']
    ...
    def _generate_pdf(self, params):
        ...
        layer_pdf, layers = model_gen.pdfNN_layer_generator(
                nodes = nodes_per_layer, activations = activation_per_layer, layer_type = layer_type,
                flav_info = self.flavinfo, seed = self.NNseed, initializer_name = initializer, dropout = dropout
                )
    ...
    def _model_compilation(self, params):
        ...
        training_model.compile(
                optimizer_name = opt_name, learning_rate = learning_rate, loss = loss_list
            )
    ...
    def _train_and_fit(self, validation_object, epochs):
        ...
        for epoch in range(epochs):
            out = training_model.fit(
                        x = None, y = exp_list, epochs = 1, verbose=False, shuffle=False
                        )
