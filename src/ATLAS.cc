...
void ATLASZHIGHMASS49PBFilter::ReadData()
{
  ...
  for (int i = 0; i < fNData; i++)
  {
    ...
    fSys[i][fNSys-1].name = "ATLASZHIGHMASSLUMI";
...
/**
 * ATLASTTBARRAP11 data
 *
 * data and covariance matrix taken from paper:
 *                 http://arxiv.org/abs/1407.0371
 *
 */
void ATLASTTBARRAP11Filter::ReadData()
{
  /*
  // Opening files
  fstream f1, f2;
  ...
  stringstream datafile2("");
  datafile2 << dataPath() << "rawdata/"
  << fSetName << "/ATLASTTBARRAP11.covmat";
  f2.open(datafile2.str().c_str(), ios::in);
  ...
  for (int i = 0; i < fNData; i++)
  {
    ...
    // Data are normalized to the total cross section,
    f1 >> fKin1[i] >> mbin[i] >> mbin[i+1] >> fData[i] >> fStat[i] >> systot;
    fKin2[i] = ;
    fKin3[i] = 7E3;
  }
